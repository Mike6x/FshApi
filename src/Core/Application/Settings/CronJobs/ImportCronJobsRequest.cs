...
public class ImportCronJobsRequest : IRequest<int>
{
    public FileUploadRequest ExcelFile { get; set; } = default!;
}

public class ImportCronJobsRequestHandler(
    IDapperRepository repository,
    IExcelReader excelReader,
    IStringLocalizer<ImportCronJobsRequestHandler> localizer) : IRequestHandler<ImportCronJobsRequest, int>
{
    ...
    public async Task<int> Handle(ImportCronJobsRequest request, CancellationToken cancellationToken)
    {
        var items = await _excelReader.ToListAsync<CronJob>(request.ExcelFile, FileType.Excel);
